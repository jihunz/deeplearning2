"""English figures (tiny-budget-ada style: Pretendard, Transformer=blue/LSTM=gray, direct labels + insight)."""
...
a1.text(0.15,0.66,'validation',color=RED,fontsize=10,va='center',ha='left',fontweight='bold')
a1.annotate("val loss explodes = overfitting",xy=(6,1.99),xytext=(2.3,1.83),fontsize=10.5,color=RED,
...
a2.annotate("val stable = generalizes",xy=(7,0.33),xytext=(3.0,0.40),fontsize=10.5,color=BLUE,
...
fig.tight_layout(pad=1.2); fig.savefig(R+"/fig_loss_en.png",dpi=200,bbox_inches='tight',facecolor='white'); print("fig_loss_en")
...
fig.tight_layout(pad=1.2); fig.savefig(R+"/fig_confusion_en.png",dpi=200,bbox_inches='tight',facecolor='white'); print("fig_confusion_en")

# ── fig_ablation
pct=[5,25,50,100]; lstm_a=[0.727,0.846,0.833,0.833]; tr_a=[0.798,0.875,0.897,0.910]
fig,ax=plt.subplots(figsize=(8,4.7))
ax.plot(pct,lstm_a,color=GRAY,marker='o',ms=7,lw=2.4); ax.plot(pct,tr_a,color=BLUE,marker='s',ms=7,lw=2.4)
...
for x,y in zip(pct,lstm_a): ax.text(x,y-0.017,f"{y:.3f}",color=GRAY,fontsize=9.5,ha='center',va='top')
for x,y in zip(pct,tr_a): ax.text(x,y+0.012,f"{y:.3f}",color=BLUE,fontsize=9.5,ha='center',va='bottom',fontweight='bold')
ax.annotate("saturates at 25%",xy=(25,0.844),xytext=(40,0.79),fontsize=11,color=GRAY,
...
fig.tight_layout(pad=1.2); fig.savefig(R+"/fig_ablation_en.png",dpi=200,bbox_inches='tight',facecolor='white'); print("fig_ablation_en")
...
panels=[(axs[0],lt,lv,"LSTM gradient saliency","predicted: Sci/Tech (wrong)",GRAY,'giddy'),
        (axs[1],st,sv,"Transformer gradient saliency","predicted: Sports (correct)",BLUE,None),
        (axs[2],at,av,"Transformer attention weight","predicted: Sports (correct)",BLUE,None)]
for ax,toks,vals,title,sub,c,hl in panels:
    ...
    tc=RED if 'wrong' in sub else c
    ax.set_title(f"{title}\n{sub}",fontsize=11,color=tc,fontweight='bold',loc='left',pad=10,linespacing=1.5)
    clean(ax); ax.tick_params(axis='x',labelsize=9)
axs[0].text(0.232,7,"off-topic",color=RED,fontsize=9.5,va='center',ha='left',fontweight='bold')
fig.suptitle("Same Sports article: LSTM fixates on one token (wrong); Transformer distributes over topical tokens",
    fontsize=12.5,color=INK,fontweight='bold',y=1.14,x=0.5)
fig.tight_layout(pad=1.0); fig.savefig(R+"/fig_mechanism_en.png",dpi=200,bbox_inches='tight',facecolor='white'); print("fig_mechanism_en")
